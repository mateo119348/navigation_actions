package com.example.pruebaconceptonavigationmanager.flowEngine.actions.mappers.mla
...
import com.example.pruebaconceptonavigationmanager.activities.calculator.mla.CalculatorActivityMLA
...
class ActionsMapperMLA : ActionsMapperBase() {

    override fun getActionWrapper(ruleAction: RuleAction, fieldMapper: FieldMapper): ActionWrapper {
        val fields = fieldMapper.getFields(ruleAction.fields)

        return when (ruleAction.id) {
            CALCULATOR.name -> ActionWrapper(CALCULATOR, fields, CalculatorActivityMLA::class.java)
